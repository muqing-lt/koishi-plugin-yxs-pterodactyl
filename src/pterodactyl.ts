...
export const Config = Schema.object({
  ...
  commands: Schema.object({
    bind: Schema.string().description('绑定账号指令').default('yxs.bind'),
    unbind: Schema.string().description('解绑账号指令').default('yxs.unbind'),
    start: Schema.string().description('启动服务器指令').default('yxs.start'),
    stop: Schema.string().description('停止服务器指令').default('yxs.stop'),
    restart: Schema.string().description('重启服务器指令').default('yxs.restart'),
    kill: Schema.string().description('强制终止服务器指令').default('yxs.kill'),
    status: Schema.string().description('查看服务器状态指令').default('yxs.status'),
...
interface BindingData { [qq: string]: string }
const STORAGE_PATH = path.resolve(__dirname, '../data.json');
...
namespace Storage {
  export function init() {
    try {
      ...
      if (!fs.existsSync(STORAGE_PATH)) fs.writeFileSync(STORAGE_PATH, JSON.stringify({}, null, 2), 'utf8');
      return true;
    } catch (error) {
  ...
  export function get(): BindingData {
    try {
      if (!fs.existsSync(STORAGE_PATH)) init();
  ...
  export function set(data: BindingData): boolean {
    try {
  ...
  export function getToken(qq: string): string | undefined {
    return get()[qq];
  }

  export function saveToken(qq: string, token: string): boolean {
    const data = get();
    data[qq] = token;
    return set(data);
  }

  export function removeToken(qq: string): boolean {
    const data = get();
    delete data[qq];
    return set(data);
  }

  export function hasBinding(qq: string): boolean {
    return !!get()[qq];
  }
}

const StatusMap: Record<string | null | undefined, string> = {
  'offline': '🔴 离线',
  'online': '🟢 在线',
  'starting': '🟡 启动中',
  'stopping': '🟡 停止中',
...
export function apply(ctx: Context, config: ConfigType) {
  ...
  const getWebSocketRealStatus = async (clientToken: string, serverUuid: string): Promise<string | null> => {
    return new Promise(async (resolve) => {
      ...
      try {
        const wsCredential = await axios.get(`${config.panelUrl}/api/client/servers/${serverUuid}/websocket`, {
          headers: {
        ...
        ctx.logger.error(`[YxsPteroPlugin] 获取 WebSocket 凭证失败：${(error as Error).message}`);
        clearTimeout(timeoutTimer);
        ws?.close();
        resolve(null);
      }
  ...
  const checkGroupPermission = (groupId: string): boolean => {
  ...
  const request = async <T>(
    ...
    try {
      ...
      if (axiosError.response) {
        ...
        const responseData = axiosError.response.data as { errors?: Array<{ code?: string }> };
        if (status === 401) throw new Error('❌ 认证失败：Token 无效/过期或前缀错误（需 ptlc_）。');
        if (status === 403) throw new Error(`❌ 权限不足：请勾选 servers:read、websocket.connect 权限。`);
        if (status === 404) throw new Error(`❌ 路由不存在：确认面板 URL 和服务器 UUID 正确。`);
        if (status === 400 && responseData.errors?.[0]?.code === 'ConflictingServerStateException') {
          throw new Error('❌ 服务器状态冲突：可能未启动或正在执行其他操作。');
        }
        if (status >= 500) throw new Error('❌ 面板内部错误：联系管理员或稍后再试。');
  ...
  const getUserServers = async (clientToken: string) => {
    const data = await request<any>(clientToken, 'GET', '');
    return data.data || [];
  };

  const getServerDetails = async (clientToken: string, serverUuid: string) => {
    const data = await request<any>(clientToken, 'GET', `/servers/${serverUuid}`);
    return data.attributes;
  };

  const getServerResources = async (clientToken: string, serverUuid: string) => {
    const data = await request<any>(clientToken, 'GET', `/servers/${serverUuid}/resources`);
    return data.attributes.resources;
  };

  const sendPowerSignal = async (clientToken: string, serverUuid: string, signal: 'start' | 'stop' | 'restart' | 'kill') => {
    await request(clientToken, 'POST', `/servers/${serverUuid}/power`, { signal });
  };

  const getRealServerStatus = async (
    clientToken: string,
    serverUuid: string,
    ...
    if (
      (mappedStatus === '⚫ 待初始化' || mappedStatus === '❓ 未知状态') &&
      resources &&
  ...
  const handleServerAction = async (argv: any, action: 'start' | 'stop' | 'restart' | 'kill' | 'view') => {
    const { session } = argv;
    const groupId = session.groupId;
    const qq = session.userId;

    if (!checkGroupPermission(groupId)) return '❌ 当前群聊不允许使用此指令';
    ...
    try {
      ...
      if (servers.length === 0) return `❌ 你的 Token 未绑定任何服务器。`;
      ...
      if (!reply) return '⏰ 操作超时，请重新发送指令并及时选择服务器';
      ...
      switch (action) {
        case 'view': {
          ...
          try {
            ...
            ctx.logger.warn(`[YxsPteroPlugin] 获取服务器详情失败，降级使用列表信息：${(detailError as Error).message}`);
          }
  ...
  ctx.command(config.commands.bind, '绑定翼龙 Client API Token')
    ...
    .action(async (argv, clientToken) => {
      ...
      if (!clientToken) return `❌ 请提供 Client API Token\n格式：\`${config.commands.bind} <Token>\``;
      if (!clientToken.startsWith('ptlc_')) return '❌ 无效 Token：需以 ptlc_ 为前缀（请生成 Client API Token）。';

      try {
        await getUserServers(clientToken);
        const saveSuccess = Storage.saveToken(qq, clientToken);
        if (!saveSuccess) throw new Error('存储绑定信息失败，请检查文件权限。');
  ...
  ctx.command(config.commands.unbind, '解绑翼龙账号')
    .action(async (argv) => {
      ...
      if (!Storage.hasBinding(qq)) return '❌ 你尚未绑定账号';
      const removeSuccess = Storage.removeToken(qq);
      if (!removeSuccess) return '❌ 解绑失败，请检查文件权限。';
      return `✅ 成功解绑翼龙账号！`;
    });
...
export const name = 'yxs-pterodactyl';
export const description = '对接翼龙面板 Client API，支持服务器实时状态（WebSocket）、资源监控、启停/重启/强制终止（指令前缀 yxs）';
export const version = '7.0.0';
export const author = 'yuanxueshi';
